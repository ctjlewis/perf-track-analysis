...
const log = (...msgs) => console.log(...msgs.map((msg) => [msg, "\n"]).flat());
...
Array.prototype.sum = function () {
  return Number(this.reduce((a, b) => a + b));
};
...
const getFormattedTable = () => {
    ...
    for (const [framework, percentages] of Object.entries(dataTable)) {
        let i = 0;
        formattedTable[framework] = {};
        for (const percentage of percentages) {
            const range = `${ranges[i++].join('-')} kB`;
            formattedTable[framework][range] = `${percentage}%`;
        }
    }
    return formattedTable;
}

const iterateOverData = (fn) =>
  Object.entries(dataTable).map(([framework, dataSeries]) => [
    framework,
    fn(dataSeries),
  ]);
 ...
 */
const pctAboveKilobyteThreshold = (threshold = 0) =>
  iterateOverData((values) => {
    const included = ranges.filter(([lower, upper]) => lower >= threshold);

    const sliceIndex = ranges.length - included.length;
    return Number(values.slice(sliceIndex).sum().toFixed(2));
  });

/**
 * Get the lower bound on the JS size averages by framework (kB)
 */
const avgJSKilobytes = () =>
  iterateOverData((percentages) => {
    let i = 0;
    const weights = [];
...
const totalJSGigabytes = () => {
  let i = 0;
  const avgs = avgJSKilobytes().map(([framework, average]) => {
    const { sampleSize } = sourceData[framework];
    return (average * sampleSize.framework) / 1e6;
  });

  return iterateOverData((values) => Number(avgs[i++].toFixed(2)));
};

const formatted = getFormattedTable();

/**
 * Log the formatted data so it's human-readable.
 */
log(formatted);
